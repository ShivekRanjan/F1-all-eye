...
| **A from-scratch transformer vs regex — and the sample said "can't tell"** — built a 450k-parameter intent+slot transformer (own tokeniser, attention, training loop) to beat a hand-written parser. Across 3 seeds the rules win slot F1 3/3 (0.845 vs 0.626–0.717, precision 1.000) and the transformer wins intent 3/3 — but the seed spread (0.091–0.161) is the same size as the gaps, so the ship decision splits 2/3–1/3. Rules ship on the metric that matters; the tie is reported rather than broken | `analysis/nlu_benchmark.py`, METHODOLOGY §15 |
